...
function getFullNames(/* CODE HERE */runners) {
  ...
  runners.forEach(returnArray)

  function returnArray(name){
...
function tallyUpDonations(/* CODE HERE */runners) {
  /* CODE HERE */
  if(runners.length > 0)
    return (runners.map(index => index.donation)).reduce((a, b) => a + b)
  else
    return 0
}
 ...
 * etc
*/
function counterMakerWithLimit(/* CODE HERE */counterMakerWithLimit) {
  /* CODE HERE */
  let count = -1;
  
  function counter() {
    if(count < counterMakerWithLimit){
      ++count
      return count
    } else if(count === counterMakerWithLimit)
      return count = 0
    else{
      --count
      return count
